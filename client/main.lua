	...
	RDX.TriggerServerCallback('rdx_billing:getBills', function(bills)
		...
		for i=1, #bills, 1 do
			table.insert(elements, {
				label  = ('%s - <span style="color:red;">%s</span>'):format(bills[i].label, _U('invoices_item', RDX.Math.GroupDigits(bills[i].amount))),
				billID = bills[i].id
			})
		end
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(0)
		if IsControlJustReleased(0, Keys['F1']) and not isDead and not RDX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'billing') then
			ShowBillsMenu()
		end
	end
end)
